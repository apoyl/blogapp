...
static const CGFloat kHeightOfTopScrollView = 38.0f; //44
static const CGFloat kWidthOfButtonMargin = 12.0f;
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [vc.title sizeWithFont:[UIFont systemFontOfSize:kFontSizeOfTabButton] constrainedToSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView) lineBreakMode:NSLineBreakByTruncatingTail];
    
      //   vc.title boundingRectWithSize:<#(CGSize)#> options:<#(NSStringDrawingOptions)#> attributes:<#(NSDictionary *)#> context:<#(NSStringDrawingContext *)#>
        ...
        if (i == 0) {
            CGFloat f=12.0f;
            //累计每个tab文字的长度
            topScrollViewContentWidth +=f+textSize.width;
            //设置按钮尺寸
            [button setFrame:CGRectMake(f,0,
                                        textSize.width, kHeightOfTopScrollView)];
            //计算下一个tab的x偏移量
            xOffset += textSize.width + f;
            _shadowImageView.frame = CGRectMake(f, 0, textSize.width, _shadowImage.size.height);
            button.selected = YES;
        }else{
            //累计每个tab文字的长度
